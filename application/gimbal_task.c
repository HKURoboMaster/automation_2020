...
int gimbal_manual_handle_debug_flag = 1;
// Edited By Eric Chen
...
// Aimming related
extern float auto_aiming_pitch[2];
extern float auto_aiming_yaw[2];
...
void gimbal_task(void const *argument)
{
  ...
  while (1)
  {
    ...
    if (gimbal_manual_handle_debug_flag == 1) //Edited by Yemi: manual handle mode for debugging
    {
      if (rc_device_get_state(prc_dev, RC_S2_UP) == RM_OK || rc_device_get_state(prc_dev, RC_S2_MID) == RM_OK || rc_device_get_state(prc_dev, RC_S2_MID2UP) == RM_OK || rc_device_get_state(prc_dev, RC_S2_UP2MID == RM_OK))
      {
        ...
        if (prc_info->kb.bit.X != 1)
        {
          ...
          pc_js1 = (int)(auto_aiming_yaw[0] * 1000); // For debug purpose
          ...
          if (time_pc != time_last)
          {
            ...
            yaw_speed_raw = auto_aiming_yaw[1]; // Unit Degree Per second
            pit_speed_raw = auto_aiming_pitch[1];
            // Unit Degree per second
            ref_yaw_speed = pgimbal->sensor.rate.yaw_rate; // Contemporarily not used
          ...
          if (prc_info->mouse.r || rc_device_get_state(prc_dev, RC_S2_UP) == RM_OK)
          {
            ...
            // #ifdef ACC_KALMAN  // Reserved for debugging purpose
            ...
            if (pc_counter == 200)
            {
              ...
              // try to prevent the angle control from shaking
