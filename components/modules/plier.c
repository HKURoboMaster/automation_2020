...
int32_t plier_cascade_register(struct plier *plier, char *name, enum device_can can)
{
  ...
  pid_struct_init(&(plier->cascade.outer), 500, 600, 15, 0, 0);
  pid_struct_init(&(plier->cascade.inter), 30000, 3000, 240, 0, 0);
...
static int32_t plier_ecd_input_convert(struct controller *ctrl, void *input)
{
  ...
  cascade_fdb->inter_fdb = -data->ecd_angle;
  return RM_OK;
}
...
int32_t plier_execute(struct plier *plier)
{
  ...
  motor_device_set_current(&(plier->motor[PLIER_MOTOR_INDEX_R]), -(int16_t)PLIER_MOTOR_POSITIVE_DIR * motor_out);
